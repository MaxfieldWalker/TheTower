...
public class KinectWrapper
{
    ...
    {
        if (GetJointPositionNative(userID, joint, ref pPosition)) return true;
    ...
    {
        if (GetJointOrientationNative(userID, joint, ref pOrientation)) return true;
    ...
    // 必要なdllなどの存在をチェックしてプロジェクトフォルダにコピーする
    public static bool CheckOpenNIPresence()
    {
        bool bAllCopied = true;

        // OPENNI2のdllやiniはあるか?
        bAllCopied = checkOpenNI2();
        if (!bAllCopied) return false;

        // OPENNI2のdllやiniはあるか?
        bAllCopied = checkNite2();
        if (!bAllCopied) return false;

        bAllCopied = checkUnityInterface2();
        if (!bAllCopied) return false;

        bAllCopied = checkMsvcp100();
        if (!bAllCopied) return false;

        bAllCopied = checkMsvcr100();
        if (!bAllCopied) return false;

        return bAllCopied;
    }

    private static bool checkOpenNI2()
    {
        bool bOneCopied = false;
        bool bAllCopied = true;
        bool bArchX64 = Is64bitArchitecture();

        // 環境変数からOPENNI2の場所を取得する
        string sOpenNIPath = System.Environment.GetEnvironmentVariable(!bArchX64 ? "OPENNI2_REDIST" : "OPENNI2_REDIST64");
        // OPENNI2はインストールされているか
    ...
    private static bool checkNite2()
    {
        bool bOneCopied = false;
        bool bAllCopied = true;
        bool bArchX64 = Is64bitArchitecture();
    ...
    private static bool checkUnityInterface2()
    {
        bool bOneCopied = false;
        bool bAllCopied = true;
        bool bArchX64 = Is64bitArchitecture();
    ...
    private static bool checkMsvcp100()
    {
        bool bOneCopied = false;
        bool bAllCopied = true;
        bool bArchX64 = Is64bitArchitecture();
    ...
    private static bool checkMsvcr100()
    {
        bool bOneCopied = false;
        bool bAllCopied = true;
        bool bArchX64 = Is64bitArchitecture();
